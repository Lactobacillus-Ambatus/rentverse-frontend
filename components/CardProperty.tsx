    ...
    <div className={clsx([
      ...
      <Link href={`/listing/${id}`} className="block group">
        ...
        <div className="relative">
          ...
          {/* Property Type Badge */}
          <div className="absolute top-4 left-4 bg-white/90 backdrop-blur-sm px-3 py-1.5 rounded-lg flex items-center gap-2 text-sm font-medium text-slate-700">
        ...
        <div className="p-5">
          ...
          {/* Title */}
          <h3 className="text-lg font-semibold text-slate-900 mt-1 mb-3 group-hover:text-teal-600 transition-colors">
            {title}
          </h3>
          ...
          <div className="flex items-center justify-between">
            {/* Price */}
            <span className="text-xl font-bold text-orange-500">{getLocaledPrice(price)}</span>
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-1 text-slate-600">
                ...
                <span className="text-sm font-medium">{getLocaledArea(area)}</span>
              </div>

              {/* Rating */}
              <div className="flex items-center gap-1 text-slate-600">
                <Star size={16} className="fill-yellow-400 text-yellow-400" />
                <span className="text-sm font-medium">{getLocaledRating(rating)}</span>
              </div>
            </div>
          </div>
        </div>
